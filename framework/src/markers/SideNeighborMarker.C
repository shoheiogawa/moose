...
defineLegacyParams(SideNeighborMarker);

InputParameters
SideNeighborMarker::validParams()
{
  InputParameters params = Marker::validParams();
  params.addClassDescription("A marker that marks elements within 'depth' "
                             "elements from selected side sets in selected blocks.");
  params.addRequiredParam<unsigned int>(
      "depth", "The number of elements refined from the side sets");
  params.addRequiredParam<std::vector<BoundaryName>>(
      "sidesets", "The names of the side sets with which this marker marks neighbor elements");
  MooseEnum marker_states = Marker::markerStates();
  params.addRequiredParam<MooseEnum>(
      "mark", marker_states, "How to mark elements near the side sets");
                              ...
                              unsigned int previous_elem_id, unsigned int depth)
{
  ...
  // `depth` is equal to the nubmer of elements from the side sets to be refined.
  // If it is 1, you don't go to any neighbor elements for the side set search.
  if (depth > 1)
  {
    for (unsigned int side = 0; side < elem->n_sides(); side++)
    {
